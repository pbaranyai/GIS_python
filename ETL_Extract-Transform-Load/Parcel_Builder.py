...
AIRPARCEL_AST = AST + "\\CCSDE.AST.Crawford_Parcels\\CCSDE.AST.TaxParcels_Air"
AIRPARCEL_AUTOWKSP = AUTOWORKSPACE_AST + "\\CCSDE.AUTO_WORKSPACE.TaxParcel_Air_Joined"
...
Building_Only_Temp = AUTOWORKSPACE_AST + "\\Building_Only_Temp"
...
print ("Tax Parcels - Air Feature Class")
...
write_log("Tax Parcels - Air Feature Class", logfile)
...
print ("\n Creating Tax_Parcel_Air_Joined - AUTOWORKSPACE from Tax_Parcel_Air - AST & VISION tables")
write_log("\n Creating Tax_Parcel_Air_Joined - AUTOWORKSPACE from Tax_Parcel_Air - AST & VISION tables", logfile)

try:
    # Delete rows from Tax_Parcel_Air_Joined
    arcpy.DeleteRows_management(AIRPARCEL_AUTOWKSP)
except:
    print ("\n Unable to delete rows from Tax_Parcel_Air_Joined")
    write_log("\n Unable to delete rows from Tax_Parcel_Air_Joined", logfile)
    logging.exception('Got exception on delete rows from Tax_Parcel_Air_Joined logged at:' + str(Day) + " " + str(Time))
    raise
    sys.exit ()

try:   
    # Make temporary feature class from Tax Parcels - Air (creates temporary file of tax parcels - air from assessment workspace, filtering out Status = Pending)
    AirParcel_Temp = arcpy.conversion.FeatureClassToFeatureClass(AIRPARCEL_AST, AUTOWORKSPACE_AST, "AirParcel_Temp", "Type <> 'Pending'", r'PID "PID Number" true true false 4 Long 0 10,First,#,'+AIRPARCEL_AST+',PID,-1,-1;CAMA_PIN "MBLU (Map Block Lot Unit) #" true true false 100 Text 0 0,First,#,'+AIRPARCEL_AST+',CAMA_PIN,0,100;Type "Type of air parcel" true true false 255 Text 0 0,First,#,'+AIRPARCEL_AST+',Type,0,255;Elevation "Elevation (in feet)" true true false 4 Long 0 10,First,#,'+AIRPARCEL_AST+',Elevation,-1,-1;Building "Building designation (if needed)" true true false 255 Text 0 0,First,#,'+AIRPARCEL_AST+',Building,0,255;Floor "Floor (if needed)" true true false 100 Text 0 0,First,#,'+AIRPARCEL_AST+',Floor,0,100;Unit "Unit (if needed)" true true false 100 Text 0 0,First,#,'+AIRPARCEL_AST+',Unit,0,100;Landex_URL_Choice "Landex URL (choose one)" true true false 100 Text 0 0,First,#,'+AIRPARCEL_AST+',Landex_URL_Choice,0,100;created_user "created_user" false true false 255 Text 0 0,First,#,'+AIRPARCEL_AST+',created_user,0,255;created_date "created_date" false true false 8 Date 0 0,First,#,'+AIRPARCEL_AST+',created_date,-1,-1;last_edited_user "last_edited_user" false true false 255 Text 0 0,First,#,'+AIRPARCEL_AST+',last_edited_user,0,255;last_edited_date "last_edited_date" false true false 8 Date 0 0,First,#,'+AIRPARCEL_AST+',last_edited_date,-1,-1', '')
except:
    print ("\n Unable to make temporary feature class from Tax Parcels - Air")
    write_log("\n Unable to make temporary feature class from Tax Parcels - Air", logfile)
    logging.exception('Got exception on make temporary feature class from Tax Parcels - Air logged at:' + str(Day) + " " + str(Time))
    raise
    sys.exit ()

try:   
    # Join AirParcel_Temp to VISIDATA_TEMP (joins Tax Parcels - Air from assessment workspace to vision data)
    arcpy.management.JoinField(AirParcel_Temp, "PID", VISIDATA_TEMP, "REM_PID", "REM_USRFLD;REM_BLDG_NAME;REM_MBLU_BLOCK;REM_MBLU_BLOCK_CUT;PRC_CENSUS;PRC_CMPLX_DESC;REM_CMPLX_NAME;PRC_CMPLX_PID;REM_USRFLD_DESC;SLH_BOOK;SLH_PAGE;LND_DSTRCT;REM_INET_SUPPRESS;REM_INET_SUPPRESS_1;REM_IS_CONDO_MAIN;LND_USE_CODE;LND_USE_DESC;REM_LAST_UPDATE;REM_LEGAL_AREA;REM_MBLU_LOT;REM_MBLU_LOT_CUT;MAD_ID;REM_MBLU_MAP;REM_MBLU_MAP_CUT;MUNI_NAME;REM_ALT_PRCL_ID;OWN_ID;OWN_LINE;OWN_NAME1;OWN_NAME2;REM_OWN_NAME;REM_PRCL_LOCN;REM_PRCL_LOCN_CITY;REM_PRCL_LOCN_STT;REM_PRCL_LOCN_ZIP;REM_PRCL_STATUS_DATE;REM_PARCEL_STATUS;PRC_VALUE;REM_PID;PID_TEXT;REM_MNC;ROW_CREATE_DATE;ROW_LINE_NUM;ROW_MAD_ID;ROW_MAD_ISPRIMARY;ROW_OWN_ID;ROW_OWN_PCT;ROW_PID;ROW_PRIMARY;SLH_SALE_DATE;SLH_PRICE;SLH_CURRENT_OWNER;SLH_LINE_NUM;SLH_PID;PRC_PF_LOCN_DESC;PRC_PF_LOCN;REM_STATUS_DATE;MAD_MAIL_ADDR1;MAD_MAIL_ADDR2;MAD_MAIL_CITY;MAD_MAIL_NAME1;MAD_MAIL_NAME2;MAD_MAIL_STATE;MAD_MAIL_ZIP;PRC_TTL_ASSESS;PRC_TTL_ASSESS_BLDG;PRC_TTL_ASSESS_IMPROVEMENTS;PRC_TTL_ASSESS_LND;PRC_TTL_MRKT_ASSESS;PRC_TTL_ASSESS_OB;REM_MBLU_UNIT;REM_MBLU_UNIT_CUT;REM_PIN;REM_USE_CODE;PRC_USRFLD_10;PRC_USRFLD_09")
    print ("  VISIDATA_TEMP joined to AirParcel_Temp...")
    write_log("  VISIDATA_TEMP joined to AirParcel_Temp...", logfile)
except:
    print ("\n Unable to join AirParcel_Temp to VISIDATA_TEMP")
    write_log("\n Unable to join AirParcel_Temp to VISIDATA_TEMP", logfile)
    logging.exception('Got exception on join AirParcel_Temp to VISIDATA_TEMP logged at:' + str(Day) + " " + str(Time))
    raise
    sys.exit ()

try:   
    # Append from AirParcel_Temp to TaxParcel_Air_Joined (appends temporary file of Tax Parcel - Air FC and vision data to "real" feature class)
    arcpy.management.Append(AirParcel_Temp, AIRPARCEL_AUTOWKSP, "NO_TEST", r'PID "PID Number" true true false 4 Long 0 10,First,#,'+AIRPARCEL_AUTOWKSP+',PID,-1,-1;CAMA_PIN "MBLU (Map Block Lot Unit) #" true true false 100 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',CAMA_PIN,0,100;Type "Type of air parcel" true true false 255 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',Type,0,255;Elevation "Elevation (in feet)" true true false 4 Long 0 10,First,#,'+AIRPARCEL_AUTOWKSP+',Elevation,-1,-1;Building "Building designation (if needed)" true true false 255 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',Building,0,255;Floor "Floor (if needed)" true true false 100 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',Floor,0,100;Unit "Unit (if needed)" true true false 100 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',Unit,0,100;Landex_URL_Choice "Landex URL (choose one)" true true false 100 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',Landex_URL_Choice,0,100;created_user "created_user" true true false 255 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',created_user,0,255;created_date "created_date" true true false 8 Date 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',created_date,-1,-1;last_edited_user "last_edited_user" true true false 255 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',last_edited_user,0,255;last_edited_date "last_edited_date" true true false 8 Date 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',last_edited_date,-1,-1;REM_MNC "REM_MNC" true true false 4 Long 0 10,First,#,'+AIRPARCEL_AUTOWKSP+',REM_MNC,-1,-1;REM_PID "PID Number" true true false 4 Long 0 10,First,#,'+AIRPARCEL_AUTOWKSP+',REM_PID,-1,-1;REM_PIN "UPI Number" true true false 35 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_PIN,0,35;REM_OWN_NAME "Owner Name" true true false 85 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_OWN_NAME,0,85;REM_PRCL_LOCN "Parcel Location" true true false 50 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_PRCL_LOCN,0,50;REM_PRCL_LOCN_CITY "Parcel Location City" true true false 18 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_PRCL_LOCN_CITY,0,18;REM_PRCL_LOCN_STT "Parcel Location State" true true false 2 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_PRCL_LOCN_STT,0,2;REM_PRCL_LOCN_ZIP "Parcel Location Zipcode" true true false 12 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_PRCL_LOCN_ZIP,0,12;REM_ALT_PRCL_ID "Old IBM Number" true true false 35 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_ALT_PRCL_ID,0,35;REM_PRCL_STATUS_DATE "Parcel Status date - CAMA Software" true true false 8 Date 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_PRCL_STATUS_DATE,-1,-1;REM_MBLU_MAP "Map" true true false 7 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_MBLU_MAP,0,7;REM_MBLU_MAP_CUT "Map Cut - Not Used" true true false 3 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_MBLU_MAP_CUT,0,3;REM_MBLU_BLOCK "Block" true true false 7 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_MBLU_BLOCK,0,7;REM_MBLU_BLOCK_CUT "Block Cut - Not Used" true true false 3 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_MBLU_BLOCK_CUT,0,3;REM_MBLU_LOT "Lot" true true false 7 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_MBLU_LOT,0,7;REM_MBLU_LOT_CUT "Lot Cut - Not Used" true true false 3 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_MBLU_LOT_CUT,0,3;REM_MBLU_UNIT "Unit" true true false 7 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_MBLU_UNIT,0,7;REM_MBLU_UNIT_CUT "Unit Cut - Not Used" true true false 3 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_MBLU_UNIT_CUT,0,3;REM_STATUS_DATE "Status Date - CAMA software" true true false 8 Date 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_STATUS_DATE,-1,-1;REM_INET_SUPPRESS "Internet Suppression" true true false 4 Long 0 10,First,#,'+AIRPARCEL_AUTOWKSP+',REM_INET_SUPPRESS,-1,-1;REM_IS_CONDO_MAIN "Is Condo Main Parcel" true true false 2 Short 0 5,First,#,'+AIRPARCEL_AUTOWKSP+',REM_IS_CONDO_MAIN,-1,-1;REM_CMPLX_NAME "Complex Name" true true false 30 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_CMPLX_NAME,0,30;REM_BLDG_NAME "Acreage and Description" true true false 60 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_BLDG_NAME,0,60;REM_USE_CODE "Use Code" true true false 4 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_USE_CODE,0,4;REM_LEGAL_AREA "Legal Area" true true false 8 Double 8 38,First,#,'+AIRPARCEL_AUTOWKSP+',REM_LEGAL_AREA,-1,-1;REM_LAST_UPDATE "Last Update - CAMA software" true true false 8 Date 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_LAST_UPDATE,-1,-1;REM_USRFLD "6 digit PID" true true false 6 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_USRFLD,0,6;REM_USRFLD_DESC "Control Number" true true false 40 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_USRFLD_DESC,0,40;PID_TEXT "PID Text format" true true false 15 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',PID_TEXT,0,15;REM_PARCEL_STATUS "Parcel Status in CAMA" true true false 1 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',REM_PARCEL_STATUS,0,1;LND_USE_CODE "Land Use Code" true true false 4 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',LND_USE_CODE,0,4;LND_USE_DESC "Land Use Description" true true false 40 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',LND_USE_DESC,0,40;LND_DSTRCT "District Number" true true false 6 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',LND_DSTRCT,0,6;MUNI_NAME "Municipality Name" true true false 75 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',MUNI_NAME,0,75;PRC_PF_LOCN "School District Code" true true false 15 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',PRC_PF_LOCN,0,15;PRC_PF_LOCN_DESC "School District" true true false 50 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',PRC_PF_LOCN_DESC,0,50;PRC_USRFLD_09 "User Field 9 - Not Used" true true false 30 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',PRC_USRFLD_09,0,30;PRC_USRFLD_10 "User Field 10 - Not Used" true true false 30 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',PRC_USRFLD_10,0,30;PRC_TTL_ASSESS_BLDG "Total Building Assessment" true true false 8 Double 8 38,First,#,'+AIRPARCEL_AUTOWKSP+',PRC_TTL_ASSESS_BLDG,-1,-1;PRC_TTL_ASSESS_IMPROVEMENTS "Total Improvements Assessment" true true false 8 Double 8 38,First,#,'+AIRPARCEL_AUTOWKSP+',PRC_TTL_ASSESS_IMPROVEMENTS,-1,-1;PRC_TTL_ASSESS_LND "Total Land Assessment" true true false 8 Double 8 38,First,#,'+AIRPARCEL_AUTOWKSP+',PRC_TTL_ASSESS_LND,-1,-1;PRC_TTL_ASSESS_OB "Total Out Building Assessment" true true false 8 Double 8 38,First,#,'+AIRPARCEL_AUTOWKSP+',PRC_TTL_ASSESS_OB,-1,-1;PRC_VALUE "Parcel Value" true true false 8 Double 8 38,First,#,'+AIRPARCEL_AUTOWKSP+',PRC_VALUE,-1,-1;PRC_CMPLX_PID "Complex PID" true true false 4 Long 0 10,First,#,'+AIRPARCEL_AUTOWKSP+',PRC_CMPLX_PID,-1,-1;PRC_CMPLX_DESC "Complex Description" true true false 30 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',PRC_CMPLX_DESC,0,30;PRC_CENSUS "Census - Not Used" true true false 20 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',PRC_CENSUS,0,20;PRC_TTL_MRKT_ASSESS "Total Market Assessment" true true false 8 Double 8 38,First,#,'+AIRPARCEL_AUTOWKSP+',PRC_TTL_MRKT_ASSESS,-1,-1;PRC_TTL_ASSESS "Total Assessment" true true false 8 Double 8 38,First,#,'+AIRPARCEL_AUTOWKSP+',PRC_TTL_ASSESS,-1,-1;OWN_ID "OWN_ID" true true false 4 Long 0 10,First,#,'+AIRPARCEL_AUTOWKSP+',OWN_ID,-1,-1;OWN_NAME1 "OWN_NAME1" true true false 85 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',OWN_NAME1,0,85;OWN_NAME2 "OWN_NAME2" true true false 85 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',OWN_NAME2,0,85;ROW_PID "ROW_PID" true true false 4 Long 0 10,First,#,'+AIRPARCEL_AUTOWKSP+',ROW_PID,-1,-1;ROW_OWN_ID "ROW_OWN_ID" true true false 4 Long 0 10,First,#,'+AIRPARCEL_AUTOWKSP+',ROW_OWN_ID,-1,-1;ROW_LINE_NUM "ROW_LINE_NUM" true true false 4 Long 0 10,First,#,'+AIRPARCEL_AUTOWKSP+',ROW_LINE_NUM,-1,-1;ROW_OWN_PCT "ROW_OWN_PCT" true true false 2 Short 0 5,First,#,'+AIRPARCEL_AUTOWKSP+',ROW_OWN_PCT,-1,-1;ROW_PRIMARY "ROW_PRIMARY" true true false 2 Short 0 5,First,#,'+AIRPARCEL_AUTOWKSP+',ROW_PRIMARY,-1,-1;ROW_CREATE_DATE "ROW_CREATE_DATE" true true false 8 Date 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',ROW_CREATE_DATE,-1,-1;ROW_MAD_ID "ROW_MAD_ID" true true false 4 Long 0 10,First,#,'+AIRPARCEL_AUTOWKSP+',ROW_MAD_ID,-1,-1;ROW_MAD_ISPRIMARY "ROW_MAD_ISPRIMARY" true true false 2 Short 0 5,First,#,'+AIRPARCEL_AUTOWKSP+',ROW_MAD_ISPRIMARY,-1,-1;OWN_LINE "OWN_LINE" true true false 255 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',OWN_LINE,0,255;MAD_MAIL_NAME1 "Tax Bill Mailing Address Name 1" true true false 85 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',MAD_MAIL_NAME1,0,85;MAD_MAIL_NAME2 "Tax Bill Mailing Address Name 2" true true false 85 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',MAD_MAIL_NAME2,0,85;MAD_MAIL_ADDR1 "Tax Bill Mailing Address 1" true true false 50 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',MAD_MAIL_ADDR1,0,50;MAD_MAIL_CITY "Tax Bill Mailing Address City" true true false 30 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',MAD_MAIL_CITY,0,30;MAD_MAIL_STATE "Tax Bill Mailing Address State" true true false 20 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',MAD_MAIL_STATE,0,20;MAD_MAIL_ZIP "Tax Bill Mailing Address Zipcode" true true false 20 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',MAD_MAIL_ZIP,0,20;MAD_MAIL_ADDR2 "Tax Bill Mailing Address 2" true true false 50 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',MAD_MAIL_ADDR2,0,50;MAD_ID "Mailing Address VISION ID" true true false 4 Long 0 10,First,#,'+AIRPARCEL_AUTOWKSP+',MAD_ID,-1,-1;SLH_PID "Sales History PID" true true false 4 Long 0 10,First,#,'+AIRPARCEL_AUTOWKSP+',SLH_PID,-1,-1;SLH_LINE_NUM "Sales History Line Number" true true false 4 Long 0 10,First,#,'+AIRPARCEL_AUTOWKSP+',SLH_LINE_NUM,-1,-1;SLH_SALE_DATE "Sale Date" true true false 8 Date 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',SLH_SALE_DATE,-1,-1;SLH_BOOK "Deed Book / Instrument Number" true true false 15 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',SLH_BOOK,0,15;SLH_PAGE "Deed Page" true true false 15 Text 0 0,First,#,'+AIRPARCEL_AUTOWKSP+',SLH_PAGE,0,15;SLH_PRICE "Sale Price" true true false 8 Double 8 38,First,#,'+AIRPARCEL_AUTOWKSP+',SLH_PRICE,-1,-1;SLH_CURRENT_OWNER "Sales History Current Owner Designation" true true false 2 Short 0 5,First,#,'+AIRPARCEL_AUTOWKSP+',SLH_CURRENT_OWNER,-1,-1;REM_INET_SUPPRESS_1 "Internet Suppression" true true false 4 Long 0 10,First,#,'+AIRPARCEL_AUTOWKSP+',REM_INET_SUPPRESS_1,-1,-1', '', '')
    AirParcel_result = arcpy.GetCount_management(AIRPARCEL_AUTOWKSP)
    print ('{} has {} records'.format(AIRPARCEL_AUTOWKSP, AirParcel_result[0]))
    write_log('{} has {} records'.format(AIRPARCEL_AUTOWKSP, AirParcel_result[0]), logfile)
except:
    print ("\n Unable to append from AirParcel_Temp to TaxParcel_Air_Joined")
    write_log("\n Unable to append from AirParcel_Temp to TaxParcel_Air_Joined", logfile)
    logging.exception('Got exception on append from AirParcel_Temp to TaxParcel_Air_Joined logged at:' + str(Day) + " " + str(Time))
    raise
    sys.exit ()

try:
    # Calculate Landex URL Field in Tax_Parcel_Joined - AUTO_WORKSPACE (calculates custom URL per record for landex use based on fields from parcels)
    URLFIELDS = ['LANDEX_URL','SLH_BOOK','SLH_PAGE',str('REM_MBLU_MAP'),str('REM_MBLU_BLOCK'),str('REM_MBLU_LOT'),'PID','Landex_URL_Choice']
    cursor =  arcpy.da.UpdateCursor(AIRPARCEL_AUTOWKSP,URLFIELDS)
    for row in cursor:
        if  row[6] == None:
            continue
        if (row[7] == 'Book_Page' and row[6] > 0):
...
try:
    ...
    arcpy.Delete_management(AirParcel_Temp)
    print ("   AirParcel_Temp deleted...")
    write_log("   AirParcel_Temp deleted...",logfile)
except:
    print ("\n Unable to delete AirParcel_Temp")
    write_log("\n Unable to delete AirParcel_Temp", logfile)
    logging.exception('Got exception on delete AirParcel_Temp logged at:' + str(Day) + " " + str(Time))
    raise
    sys.exit ()

print ("       Creating Tax_Parcel_Air_Joined - AUTOWORKSPACE from Tax_Parcel_Air - AST & VISION tables completed")
write_log("       Creating Tax_Parcel_Air_Joined - AUTOWORKSPACE from Tax_Parcel_Air - AST & VISION tables completed", logfile)
